...
Data Plots
```{r}
ggplot(data = data1, aes(x = temp, y = log(area))) +
    geom_point(color = "red") +
    labs(title = "Temp vs Area")
ggplot(data = data1, aes(x = RH, y = log(area))) +
    geom_point(color = "steelblue") +
    labs(title = "Humidity vs Area")
ggplot(data = data1, aes(x = wind, y = log(area))) +
    geom_point(color = "green") +
    labs(title = "wind vs Area")
ggplot(data = data1, aes(x = rain, y = log(area))) +
    geom_point(color = "cyan") +
    labs(title = "Rain vs Area")
ggplot(data = data1, aes(x = ISI, y = log(area))) +
    geom_point(color = "darkolivegreen") +
    labs(title = "ISI vs Area")
ggplot(data = data1, aes(x = DC, y = log(area))) +
    geom_point(color = "firebrick") +
    labs(title = "DC vs Area")
ggplot(data = data1, aes(x = DMC, y = log(area))) +
    geom_point(color = "goldenrod") +
    labs(title = "DMC vs Area")
ggplot(data = data1, aes(x = FFMC, y = log(area))) +
...
summary(g1)
#plot(g1)
ggplot(data1, aes(x=temp, y=log(area))) + 
...
g2 = lm(log(area) ~ d*i*f, data1)
summary(g2)
ggplot(data1, aes(x=FFMC, y=log(area))) + 
  geom_point(color = "seagreen") +
  geom_line(aes(y=predict(g2)))
```
Model 3
```{r}
plot(log(area) ~ FFMC, data = data1, main = "data(cars)  &  smoothing splines")
...
fire_model <- lm(log(area) ~ log(temp) + log(RH) + wind + I(wind^2), data = df_fire)
summary(fire_model)
```
```{r}
df_pred <- df %>% 
